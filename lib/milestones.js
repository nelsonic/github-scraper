...
function milestones (project, callback) {
  ...
  wreck.get(url, function (error, response, html) {
    if (error || response.statusCode !== 200) {
      ...
      var e = items[0].children[0];
      // console.log(e.next.children[3].children[1]);

      $('.table-list-item').each(function (i) {
        var e = items[i].children[0];
        console.log($(this).find('.milestone-meta-item:nth-child(1)').text());
        // data.entries.push($(this).find('h2'));
        var milestone = {
          name     : e.next.children[1].children[1].children[0].data.trim(),
          due      : $(this).find('.milestone-meta-item:nth-child(1)').text().trim(),
          updated  : $(this).find('.milestone-meta-item:nth-child(2)').text().trim(),
          desc     : e.next.children[5].children[1].children[0].data.trim(),
