...
function org($, url, callback) {
  ...
  data.entries = items.map( function (i) { // JS counters start at 0.
    var parent = '.public:nth-child(' + (i+1) +') '; // CSS selectors start at 1.
    return {
      name: $(parent + ' a').first().text().trim(),
      lang: $(parent + 'span[itemprop=programmingLanguage]').first().text().trim(),
      desc: $(parent + 'p.d-inline-block').first().text().trim(),
