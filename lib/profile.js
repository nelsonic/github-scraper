...
module.exports = function profile ($, url, callback) {
  ...
  if($('li[itemprop=homeLocation]')) {
    data.location = $('li[itemprop=homeLocation]').attr('aria-label').replace("Home location: ", '')
  }
    // ;
  ...
  if($('li[itemprop=homeLocation]')) {
    data.location = $('[itemprop=homeLocation]').attr('aria-label')
